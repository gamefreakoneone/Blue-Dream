...
- Nova/Bedrock remains in the repository for the optional legacy semantic embedding and provider fallback behind `EMBEDDING_PROVIDER=bedrock`.
...
- `Blue_dream_agents/`, `Blue_dream_agents/llm/`, `Blue_dream_agents/Tools/`, and `Capture/` are Python packages (each has `__init__.py`).
...
### Non-Authoritative Validation / Demo Material
- `Blue_dream_agents/test_image_object_pipeline.py` - standalone validation script for the vision + Gemini spatial pipeline.
- Demo/proof materials are not runtime source and should only be edited for validation, documentation, or submission work.
- The pre-rebuild cleanup removed generated bytecode, stale source copies, broken unused experiments, root reference scripts, the scratch notebook, and the unused root YOLO weight. Fall detection continues to use `Capture/trained-weights/best.pt`.
...
- Required keys in `.env` for core behavior:
  ...
  - Optional for Gmail fall alerts: `FALL_ALERT_RECIPIENT_EMAIL`
...
- `Capture/camera_feed.py` reads the fall-alert recipient from `FALL_ALERT_RECIPIENT_EMAIL` and skips Gmail delivery when it is not configured.
...
- Bedrock-native access remains in the codebase for optional legacy Nova embeddings and provider fallback.
...
- `llm/settings.py` has its own custom `.env` parser that supports Windows `SET` prefix stripping; `python-dotenv` is not required.
...
- Web and mobile geofence guidance use `GET /geofence/current`; navigation is disabled with a clear configuration error when home coordinates are unavailable.
...
- The pre-rebuild repository cleanup removed tracked caches, accidental files, obsolete experiments, machine-specific literals, and client-side home-coordinate fallbacks without changing the public API contracts.
