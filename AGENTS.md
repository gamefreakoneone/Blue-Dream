...
- Default provider for the rebuild is `LLM_PROVIDER=qwen` (DashScope key in `.env` as `DASHSCOPE_API_KEY`, with `QWEN_APIKEY` accepted as a fallback name). The OpenAI profile is used only by spec 0012, apart from the offline-tested transcription fallback. Ollama is an optional local profile — **not installed on the dev machine**; never make it a prerequisite.
...
The provider client built by spec 0003 is `Blue_dream_agents/llm/client.py`: one async OpenAI-protocol client serves text, structured output, vision, URL video, embeddings, ASR, and TTS capability boundaries. `LLM_PROVIDER=qwen|openai|ollama` selects the profile; per-capability env vars override pieces (`EMBEDDING_PROVIDER`, `VIDEO_PROVIDER`, `SPATIAL_PROVIDER`, `TRANSCRIBE_PROVIDER`, `TTS_PROVIDER`). Spec 0005 completes and live-validates Qwen ASR/video wiring. Qwen video understanding runs on presigned OSS video URLs (see the OSS video bridge in `TECHNICAL_DESIGN.md`), degrading directly to Gemini full-video analysis; Gemini also remains the fallback for spatial grounding. Frame sampling is not used because long recordings require full temporal context.
