...
**This spec is explicitly skippable.** Eligibility is already satisfied: the hackathon's proof requirement is "a repo code file demonstrating use of Alibaba Cloud services and APIs," which the DashScope integration (`Blue_dream_agents/llm/client.py`, `scripts/dashscope_spike.py`) and the OSS video bridge (`Blue_dream_agents/oss_media.py`) provide. If the schedule slips, skip this spec per the cut order in `docs/FEATURE_STATUS.md` and the demo runs fully local.
...
- No OSS usage beyond the existing 0005 video bridge (no OSS-hosted static media, no OSS-backed `/storage`), no ACK/serverless, no domain/TLS (LAN-grade demo posture; note the plain-HTTP caveat).
