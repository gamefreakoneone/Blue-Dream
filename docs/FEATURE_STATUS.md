...
| 0005 | Qwen Provider | Completed | `docs/specs/0005-qwen-provider/status.md` | Commit `73de261`; 74 tests pass and the live DashScope/OSS spike passed 9/9 checks. Full-video Qwen, production ASR, correct story-book highlighting, and privacy-safe general/time routes were validated. Existing-memory semantic export was policy-blocked and remains covered by mocks; Gemini comparison and event insertion were waived by user. Video ladder: OSS-URL Qwen → full-video Gemini → partial event, with no frame sampling. |
