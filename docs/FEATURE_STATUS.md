...
| 0008 | Proactive Channel | Completed; room-agnostic amendment pending staged-video check | `docs/specs/0008-proactive-channel/status.md` | Commit `4456ff9` preserves the original live/API/UI evidence; the corrective amendment is this commit/`HEAD`. It generalizes Bedroom/Living Room environmental hazards, adds structured/boundary-safe highlighting and a no-write CLI, and passes 176 tests plus the PWA build. The user-supplied knife-video visual check remains pending; falls and geofence behavior remain unchanged. |
...
| 0011 | Qwen Submission | In progress | `docs/specs/0011-qwen-submission/status.md` | Qwen architecture SVG + 1800×1125 PNG completed and linked from the README; rewrite, video, rehearsal, checklist, tag, and submission remain pending. |
